...
The Kali Linux distribution has two [repositories](/docs/general-use/kali-apt-sources/), which are mirrored world-wide:
...
When using the default hosts listed above, you'll automatically be redirected to a mirror site which is geographically close to you, and which is guaranteed to be up-to-date. If you prefer to manually select a mirror, click on the **mirrorlist** link near each hostname above and select a mirror that suits you. You will then need to edit your `/etc/apt/sources.list.d/kali.sources` file accordingly with the chosen values.

{{% notice info %}}
IMPORTANT! Do not add additional repositories to your <a href="/docs/general-use/kali-apt-sources/"/>/etc/apt/sources.list.d/kali.sources</a> file.<br />
<br />
