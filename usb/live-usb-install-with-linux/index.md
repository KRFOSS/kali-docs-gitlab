...
3. Proceed to (carefully!) image the Kali ISO file on the USB device. We will be assuming that the ISO image you're writing is named "kali-linux-2026.1-live-amd64.iso" and is in your current working directory.

```console
user@mint:~$ file kali-linux-2026.1-live-amd64.iso
kali-linux-2026.1-live-amd64.iso: ISO 9660 CD-ROM filesystem data (DOS/MBR boot sector) 'Kali Linux amd64' (bootable)
user@mint:~$
```
...
```console
user@mint:~$ sudo dd if=kali-linux-2026.1-live-amd64.iso of=/dev/sdX conv=fsync bs=4M status=progress
```
...
```console
user@mint:~$ sudo dd if=kali-linux-2026.1-live-amd64.iso of=/dev/sdX conv=fsync bs=4M
```
...
Alt:
```console
$ sudo dd if=kali-linux-2026.1-live-amd64.iso | pv -s 2.8G | dd of=/dev/sdX conv=fsync bs=4M
$ sudo pv kali-linux-2026.1-live-amd64.iso -Yo /dev/sdX
$ sudo cp kali-linux-2026.1-live-amd64.iso /dev/sdX
$ wget -O- https://cdimage.kali.org/kali-2026.1/kali-linux-2026.1-live-amd64.iso | sudo tee /dev/sdX | sha256sum -
```
-->
