...
- Rename the file to *.iso (i.e. change the file extension from qcow2 to iso). Take a note of the file name (eg. `kali-linux-2025.2-qemu-amd64.iso`)
- Open Proxmox
...
- Locate the uploaded file by issuing a `find / -name {filename}` (eg. `find / -name kali-linux-2025.2-qemu-amd64.iso`)
- Change directory to the location of the file
- Rename file from *.iso to *.qcow2 (eg. `mv kali-linux-2025.2-qemu-amd64.iso kali-linux-2025.2-qemu-amd64.qcow2`)
- Run command `qm importdisk VM-ID {filename.qcow2} {storage}` (eg. `qm importdisk 108 kali-linux-2025.2-qemu-amd64.qcow2 local-lvm`)
- Close shell
